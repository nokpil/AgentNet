...
class Module_GAT_old(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.att_type == 'gat':
            for i in range(self.D_att_num):
                ...
                qz = [qy for _ in range(qy.shape[1])]
                z = torch.cat((torch.stack(kz, dim=-2), torch.stack(qz, dim=-3)), -1)
                w = softmax(torch.add(self.att(z).squeeze(-1), -mask))
        ...
        mu = self.mu_dec(d)
        sig = self.sig_dec(d) # divided by 10.0 due to numeircal stability
...
class Module_GAT_VC_splitV(nn.Module):
    def __init__(self, cfg_enc, cfg_att, cfg_dec, cfg_mu, cfg_sig, D_att, D_att_num, D_agent, block_type, att_type, dropout):
        super(Module_GAT_VC_splitV, self).__init__()
...
class Module_GAT_nsa(nn.Module):
    def __init__(self, cfg_enc1, cfg_enc2, cfg_att, cfg_dec, cfg_mu, cfg_sig, cfg_corr, cfg_coef, D_att, D_att_num, D_agent, block_type, att_type, dropout, act_type):
        super(Module_GAT_nsa, self).__init__()
        ...
        self.nl = 'MS'
        self.enc_att = Parameter(
                torch.ones(cfg_enc1[0], dtype=torch.float, requires_grad=True)
            )
        self.enc = cfg_Block(block_type, cfg_enc1, D_agent, self.nl, False, False)
        self.key = cfg_Block(block_type, cfg_enc2, D_agent, self.nl, False, False)
        self.query = cfg_Block(block_type, cfg_enc2, D_agent, self.nl, False, False)
        self.value = cfg_Block(block_type, cfg_enc2, D_agent, self.nl, False, False)
        if self.att_type == 'gat':
            self.att = cfg_Block(block_type, cfg_att, D_agent, self.nl, False, False)
        elif self.att_type == 'add':
            self.att1 = cfg_Block(block_type, [D_att, D_att], D_agent, 'RL', False, False)
            self.att2 = cfg_Block(block_type, [D_att, D_att], D_agent, 'RL', False, False)
            self.att3 = cfg_Block(block_type, [D_att, 1], D_agent, 'RL', False, False)
        ...
        self.dec = cfg_Block(block_type, cfg_dec, D_agent, self.nl, False, False)
        ...
        self.corr_dec = cfg_Block(block_type, cfg_corr, D_agent, self.nl, False, False)
...
class Module_GAT_AOUP_ns(nn.Module):
    ...
    def forward(self, x, hidden, cell, R_const, test = False):
        ...
        if test:
            ...
            return (mu1, sig1, corr1), (mu2, sig2, corr2), hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)
        else:
            return hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)

class Module_GAT_AOUP_P(nn.Module):
    def __init__(self, cfg_init, cfg_lstm, cfg_enc, cfg_att, cfg_dec, cfg_mu, cfg_sig, cfg_corr, cfg_coef, D_att, D_att_num, D_agent, block_type, att_type, dropout):
        super(Module_GAT_AOUP_P, self).__init__()
    ...
    def forward(self, x, hidden, cell, R_const, test = False):
        ...
        if test:
            ...
            return (mu1, sig1, corr1), hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)
        else:
            return hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)
...
class Module_GAT_AOUP_splitV(nn.Module):
    def __init__(self, cfg_init, cfg_lstm, cfg_enc, cfg_self, cfg_att, cfg_dec, cfg_mu, cfg_sig, D_att, D_att_num, D_agent, block_type, att_type, dropout):
        super(Module_GAT_AOUP_splitV, self).__init__()
...
class Module_GAT_AOUP_splitP(nn.Module):
    def __init__(self, cfg_init, cfg_lstm, cfg_enc, cfg_self, cfg_att, cfg_dec, cfg_mu, cfg_sig, D_att, D_att_num, D_agent, block_type, att_type, dropout):
        super(Module_GAT_AOUP_splitP, self).__init__()
    ...
    def forward(self, x, hidden, cell, R_const, test = False):
        ...
        if test:
            ...
            return (mu_x, sig_x), (mu_y, sig_y), hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)
        else:
            return hidden.view(batch_num, agent_num, -1), cell.view(batch_num, agent_num, -1)
...
class Module_GAT_LSTM_splitV(nn.Module):
    def __init__(self, cfg_init, cfg_lstm, cfg_enc, cfg_self, cfg_att, cfg_dec, cfg_mu, cfg_sig, D_att, D_att_num, D_agent, block_type, att_type, dropout, eval_type, sig=True, use_sample=True):
        super(Module_GAT_LSTM_splitV, self).__init__()
    ...
    def forward(self, x, hidden, cell, verbose=False):
        ...
        if self.att_type == 'gat':
            ...
            #w = softmax(torch.add(F.leaky_relu(self.att(z), 0.2).squeeze(-1), -mask))
            for i in range(self.D_att_num):
                p_list[i].append(torch.matmul(w[:,:,:,i], v[:, :, head_dim * i: head_dim * (i + 1)]))
                #p_list[i].append(torch.matmul(w[:,:,:,i], v))
        ...
        fixed_var = 5e-3
        d_x = self.dec_x(c_list[0])
        mu_x = self.mu_dec_x(d_x).squeeze() / self.reg_norm
        if self.sig:
            sig_x = torch.sigmoid(self.sig_dec_x(d_x)).squeeze() + epsilon / self.reg_norm 

        d_y = self.dec_y(c_list[1])
        mu_y = self.mu_dec_y(d_y).squeeze() / self.reg_norm
        if self.sig:
            sig_y = torch.sigmoid(self.sig_dec_y(d_y)).squeeze() + epsilon / self.reg_norm

        d_z = self.dec_z(c_list[2])
        mu_z = self.mu_dec_z(d_z).squeeze() /self.reg_norm
        if self.sig:
            sig_z = torch.sigmoid(self.sig_dec_z(d_z)).squeeze() + epsilon / self.reg_norm

        d_vx = self.dec_vx(c_list[3])
        mu_vx = self.mu_dec_vx(d_vx).squeeze() / self.reg_norm
        if self.sig:
            sig_vx = torch.sigmoid(self.sig_dec_vx(d_vx)).squeeze() + epsilon / self.reg_norm

        d_vy = self.dec_vy(c_list[4])
        mu_vy = self.mu_dec_vy(d_vy).squeeze()/ self.reg_norm
        if self.sig:
            sig_vy = torch.sigmoid(self.sig_dec_vy(d_vy)).squeeze() + epsilon / self.reg_norm

        d_vz = self.dec_vz(c_list[5])
        mu_vz = self.mu_dec_vz(d_vz).squeeze() / self.reg_norm
        if self.sig:
            sig_vz = torch.sigmoid(self.sig_dec_vz(d_vz)).squeeze() + epsilon / self.reg_norm
